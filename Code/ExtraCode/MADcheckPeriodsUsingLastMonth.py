...
def getFinalDict(thisMonthMedianDict, lastMonthMedianDict, far):
	'''
	GIVEN TWO DICTS:
...
def getFinalDf(filesList, finalDict, far):
	...
	fileToSave = ('/').join([filesList[0].split('/')[0], filesList[0].split('/')[1], 'MAD/LastMonth',filesList[0].split('/')[3] + '_' +str(far) + '.csv'])
	print(fileToSave)
	dfList = []
...
def checkPeriodsUsingLastMonth(far):
	'''
...
for far in fars:
	print(far)
	checkPeriodsUsingLastMonth(far)
	print('---------------------------------------------------------')