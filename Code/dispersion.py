...
def calcDispersionForName(name):
		...
		try:
			...
			for f in files:
				...
				for i in range(len(durations)-1):
					...
					tempVals.append(np.log(np.mean(dx)))
	...
	print(averageDf.head())
	print(averageDf.tail())
...
def findAnomalousCommodities():
	...
	print(len(df1), len(df2), len(fullDf))
...
def findDispersionAnomalies(windowSize = 6):
	for kind in ['Price', 'Retail']:
		...
		# print(fullDf.tail(30))

		# if(commodity != 'ONION'):
		# 	continue
		# plt.plot(commodityDf['DISPERSION'], c = 'b', label = 'Actual Values')
		...
		# plt.scatter(outliers.index, outliers['DISPERSION'], c = 'r', marker = 'o', s = 120, label = 'Outliers')
		# plt.legend()
		# plt.savefig(commodity+'1.png')
