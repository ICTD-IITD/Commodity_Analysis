...
def combineForForecastedMonth():
	...
	for commodity in commodityList:
		...
		for kind in kinds:
			df = pd.DataFrame()
			files = sorted([f for f in os.listdir(folderToOpen) if (f.endswith(kind+'.csv') and (not f.startswith('Forecasted')) and (not f.startswith('Show')) and (not f.startswith('Price')) and (not f.startswith('Retail')) )])
			for file in files:
			...
			#print(files)
			states = [i[0] for i in files]
			mandis = [i[1] for i in files]
			df['STATENAME'] = states
			df['MANDINAME'] = mandis
