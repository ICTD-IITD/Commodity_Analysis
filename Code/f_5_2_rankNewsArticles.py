...
def rankNewsArticles():
	...
	newsDf =  pd.read_csv('../Data/NEWS FEED/combinedNews.csv')
	print('combined File: ', len(newsDf))
	newsDf.drop_duplicates(inplace = True)
	print('combined File after dropping duplicates: ', len(newsDf))

	newsDf = cleanData(newsDf)	
	print('after cleaning combined File: ', len(newsDf))
 
	print('MAKING TOKENS')	
	newsDf['TOKEN'] = newsDf['TEXT_CLEAN'].apply(lambda x: word_tokenize(x))
	# newsDf.to_csv('combinedNewsWithTokens.csv', index = False)
	...
	print(len(newsDf))
	newsDf.to_csv('../Data/NEWS FEED/combinedNewsRanked.csv', index = False)

# rankNewsArticles()

def rankLatestNewsArticles():
	files = [f for f in os.listdir('../Data/NEWS FEED/LATEST')]
	allCommodityDf = pd.DataFrame()
	for file in files:
		fileToOpen = os.path.join('../Data/NEWS FEED/LATEST', file)
		# print(file)
...
# APPENDING LATEST NEWS ARTICLES TO PAST NEWS ARTICLES
	...
	newsDf =  pd.read_csv('../Data/NEWS FEED/combinedNews.csv')
	print('previous news articles:', len(newsDf))
	newsDf = newsDf.append(allCommodityDf, ignore_index = True)
	print('news articles after appending', len(newsDf)) 
	newsDf.drop_duplicates(inplace = True)
	print('news articles after dropping duplicates', len(newsDf)) 
	newsDf.to_csv('../Data/NEWS FEED/combinedNews.csv', index = False)

# RANKING LATEST NEWS ARTICLES
	allCommodityDf.drop_duplicates(inplace = True)
...
# APPENDING LATEST RANKED NEWS ARTICLES TO PAST RANKED NEWS ARTICLES
	rankedNewsDf =  pd.read_csv('../Data/NEWS FEED/combinedNewsRanked.csv')
	print('ranked news articles before adding new news articles: ', len(rankedNewsDf))
	rankedNewsDf = rankedNewsDf.append(allCommodityDf, ignore_index = True)
	rankedNewsDf.drop_duplicates(inplace = True)
	print('ranked news articles after adding new news articles: ', len(rankedNewsDf))
	rankedNewsDf.to_csv('../Data/NEWS FEED/combinedNewsRanked.csv', index = False)
...
def tagNewstoMandi():
	...
	fullDf = pd.read_csv('../Data/NEWS FEED/combinedNewsRanked.csv')
	...
	finalDf.to_csv('../Data/NEWS FEED/combinedNewsRankedTagged.csv', index = False)	
